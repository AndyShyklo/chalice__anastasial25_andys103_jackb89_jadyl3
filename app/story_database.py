...
#     c.execute(command)
def createStories():
    command = "CREATE TABLE IF NOT EXISTS stories (story_id INT PRIMARY KEY, story_name text NOT NULL, authors, chapters, chapter_count)"
    c.execute(command)
...
def addChapter(story_id, content, author, date):
    ...
    command = "UPDATE stories SET chapter_count = "+ chapter_count+1 +" WHERE story_id = "+ story_id
    c.execute(command)
    db.commit()
    return story_id
...
def deleteChapter(story_id, chapter_id):
    ...
    command = "DELETE FROM chapters WHERE story_id = " + story_id + "AND" + "chapter_id = " + chapter_id
    c.execute(command)
    db.commit()
...
def deleteStory(story_id):
    ...
    # TODO
    # delete from stories
    command = "DELETE FROM stories WHERE story_id = " + story_id
    c.execute(command)
    # delete from chapters
